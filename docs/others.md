...
Publish-subscribe middlewares (so called *message brokers*) are not full
applications but assist decentralization by decoupling publishers and
subscribers at the end points.
...
They do authentication, validation, etc., and ultimately serve the queues (aka
...
filled the domains of social media, chat, and microblogging federated services.
...
Moderation can be applied locally by the user or in the servers as a
...
archives, or any kind of static blobs, while Dat is more suitable for dynamic
content such as
...
In both cases, users need to know in advance what they are looking for, being
it the exact link to a movie or a particular identity.
...
community.
In addition, following a chain requires to store posts of that chain, not of
their users, which can still be followed individually on their own `1->N`
public chain.

Aether provides peer-to-peer ephemeral public communities being more aligned
with the `N<->N` public communication of Freechains.
Posts are replicated in the network but are ephemeral in the sense that storage
is limited in a time window.
Aether requires computational proof-of-work to post to prevent SPAM in
communities.
...
The [reputation system](reps.md) tracks the quality of posts and authors in
each chain.
